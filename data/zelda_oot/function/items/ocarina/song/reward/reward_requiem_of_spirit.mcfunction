execute if entity @a[advancements={zelda_oot:song/requiem_of_spirit=false}] run return run kill @e[type=armor_stand,tag=inexistant_dummy_tag]
...
execute as @a at @s run playsound minecraft:custom.requiem_of_spirit master @a ~ ~ ~ 1 1

tag @a add warp_requiem_of_spirit
