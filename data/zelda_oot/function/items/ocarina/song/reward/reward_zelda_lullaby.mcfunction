execute if entity @a[advancements={zelda_oot:song/zelda_lullaby=false}] run return run kill @e[type=armor_stand,tag=inexistant_dummy_tag]
...
execute as @a at @s run playsound minecraft:custom.zelda_lullaby master @a ~ ~ ~ 1 1

schedule function zelda_oot:items/ocarina/song/reward/quit_ocarina 170t

say Zelda Lullaby Played
