execute if entity @a[advancements={zelda_oot:song/serenade_of_water=false}] run return run kill @e[type=armor_stand,tag=inexistant_dummy_tag]
...
execute as @a at @s run playsound minecraft:custom.serenade_of_water master @a ~ ~ ~ 1 1

tag @a add warp_serenade_of_water
