...
execute as @e[type=arrow,tag=deku_seeds,tag=!has_display] at @s run function zelda_oot:items/seeds/new_arrow
execute as @e[type=item_display,tag=seed_display] at @s unless entity @e[type=arrow,tag=has_display,distance=..1] run kill @s